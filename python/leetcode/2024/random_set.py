...
class RandomizedSetSlow:
    ...
    def getRandom(self) -> int:
        # O(n) soln: convert to list, gen rand int in range [0, len(list) - 1],
        # return item at that index
        l = [item for item in self.items]
...
class RandomizedSet:
    ...
    def getRandom(self) -> int:
        i = random.randint(0, len(self.idxs) - 1)
        return self.vals[i]
...
def test2():
    s = RandomizedSet()
    assert not s.remove(0)
    assert not s.remove(0)
    assert s.insert(0)
    assert s.getRandom() == 0
    assert s.remove(0)
    assert s.insert(0)
